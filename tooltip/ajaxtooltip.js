...
var ajaxtooltip={
	...
	useroffset: [10, -100], //additional x and y offset of tooltip from mouse cursor, respectively
	loadingHTML: '<div><img src="tooltip/loading.gif" border="0px" />  Fetching Tooltip...</div>',
...
jQuery(document).ready(function(){
	...
	$('*[@title^="ajax:"]').each(function(index){ //find all links with "title=ajax:" declaration
		...
		tooltips.push($('<div class="ajaxtooltip" id="tooltip"></div>').appendTo('body'))
		var $target=$(this)
		...
		$target.hover(
			function(e){ //onMouseover element
				...
				if (!$tooltip.get(0).loadsuccess){ //first time fetching Ajax content for this tooltip?
					...
					if ($tooltip.get(0).loadsuccess=true){
